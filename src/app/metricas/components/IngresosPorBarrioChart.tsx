...
export default function IngresosPorBarrioChart({
  items,
}: {
  items: Item[]
}) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500 sm:text-sm">
          Barrios con mayor volumen de ingresos dentro del filtro actual.
        </p>
      </div>
        ...
        {items.map((item) => (
          <div key={item.barrio}>
            <div className="relative h-10 overflow-hidden rounded-full border border-[#dbe5ef] bg-[#f4f8fb] sm:h-11">
              <div
                className="absolute inset-y-0 left-0 rounded-full bg-[linear-gradient(90deg,#d7f0ff_0%,#48b6f4_100%)]"
