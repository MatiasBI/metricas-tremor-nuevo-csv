...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await getMetricasExportRows("alumbrado", {
    ...
    const content = buildMetricasExcelContent(rows, "alumbrado")

    return new NextResponse(content, {
      headers: {
        "Content-Type": "application/vnd.ms-excel; charset=utf-8",
        "Content-Disposition": `attachment; filename="alumbrado-${today}.xls"`,
      },
    })
  } catch (error) {
    console.error(error)
